...
export default function SuggestionButton({ className = "" }) {
  ...
  useEffect(() => {
    if (!open) return;
    const onKey = (e) => {
      if (e.key === "Escape" && !submitting) closeAll();
    };
  ...
  return (
    <>
      ...
      {open &&
        createPortal(
          <div
            className="fixed inset-0 z-[70] bg-black/50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto"
            onClick={() => !submitting && closeAll()}
          >
            <div
